...
describe('todo board mobile drag navigation', () => {
  ...
  it('offers a cleanup action for all completed todos, including tagged cards', () => {
    expect(script).not.toContain('completedOwnedList')
    expect(script).not.toContain('completedTaggedList')
    ...
    expect(ko.todoBoard.messages.deleteCompletedConfirm).toBe('내가 만든 항목은 삭제되고, 태그된 항목은 내 보드에서만 제거됩니다.')
    expect(ko.todoBoard.messages.deleteCompletedSuccess).toBe('완료 항목을 정리했습니다.')
    expect(en.todoBoard.messages.deleteCompletedTitle).toContain('completed items')
    expect(en.todoBoard.messages.deleteCompletedConfirm).toBe('Items you created will be deleted. Tagged items will only be removed from your board.')
    expect(en.todoBoard.messages.deleteCompletedSuccess).toBe('Completed items cleared.')
  })

  it('captures every completed ID before confirmation', () => {
    ...
    expect(handler).not.toContain('ownedCount')
    expect(handler).not.toContain('taggedCount')
    expect(handler).not.toContain('deletedCount')
    expect(handler).not.toContain('untaggedCount')
