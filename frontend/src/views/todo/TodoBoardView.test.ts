...
describe('todo board mobile drag navigation', () => {
  ...
  it('offers a cleanup action for all completed todos, including tagged cards', () => {
    expect(script).toContain('const completedOwnedList = computed(() => doneList.value.filter((todo) => !todo.isTagged))')
    expect(script).toContain('const completedTaggedList = computed(() => doneList.value.filter((todo) => todo.isTagged))')
    expect(template).toContain('<template #header-actions>')
    expect(template).toContain('v-if="doneList.length > 0"')
    expect(template).toContain('@click="handleDeleteCompletedTodos"')
    expect(template).toContain(':disabled="isClearingCompleted"')
    expect(template).toContain('aria-label="t(\'todoBoard.actions.clearCompleted\')"')
    expect(script).toContain('BrushCleaning')
    expect(template).toContain('<BrushCleaning class="w-4 h-4" />')
    expect(script).not.toContain('Trash2')
    expect(ko.todoBoard.messages.deleteCompletedTitle).toContain('완료 항목')
    expect(ko.todoBoard.messages.deleteCompletedConfirm).toContain('{ownedCount}')
    expect(ko.todoBoard.messages.deleteCompletedConfirm).toContain('{taggedCount}')
    expect(ko.todoBoard.messages.deleteCompletedConfirm).toContain('내 보드에서만 제거')
    expect(ko.todoBoard.messages.deleteCompletedSuccess).toContain('{deletedCount}')
    expect(ko.todoBoard.messages.deleteCompletedSuccess).toContain('{untaggedCount}')
    expect(en.todoBoard.messages.deleteCompletedTitle).toContain('completed items')
    expect(en.todoBoard.messages.deleteCompletedConfirm).toContain('{ownedCount}')
    expect(en.todoBoard.messages.deleteCompletedConfirm).toContain('{taggedCount}')
    expect(en.todoBoard.messages.deleteCompletedConfirm).toContain('your board only')
    expect(en.todoBoard.messages.deleteCompletedSuccess).toContain('{deletedCount}')
    expect(en.todoBoard.messages.deleteCompletedSuccess).toContain('{untaggedCount}')
  })

  it('captures every completed ID and both category counts before confirmation', () => {
    const handlerStart = script.indexOf('async function handleDeleteCompletedTodos')
    const handler = script.slice(handlerStart, script.indexOf('\nfunction ', handlerStart + 1))

    expect(handler).toContain('const completedTodos = doneList.value')
    expect(handler).toContain('const completedTodoIds = completedTodos.map((todo) => todo.id)')
    expect(handler).toContain('const ownedCount = completedOwnedList.value.length')
    expect(handler).toContain('const taggedCount = completedTaggedList.value.length')
    expect(handler).toMatch(/completedTodoIds\.length === 0[\s\S]*?return/)
    expect(handler).toContain('await confirmDelete(')
    expect(handler).toContain('todoApi.deleteCompletedTodos(completedTodoIds)')
    expect(handler).toContain('deletedCount')
    expect(handler).toContain('untaggedCount')
    expect(handler.indexOf('const completedTodoIds')).toBeLessThan(handler.indexOf('await confirmDelete('))
    expect(handler.indexOf('await confirmDelete(')).toBeLessThan(handler.indexOf('todoApi.deleteCompletedTodos('))
    expect(handler).toContain('isClearingCompleted.value = true')
