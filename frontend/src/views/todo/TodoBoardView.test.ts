...
describe('todo board mobile drag navigation', () => {
  ...
  it('offers a destructive action for own completed todos and excludes tagged cards', () => {
    expect(script).toContain('const ownDoneList = computed(() => doneList.value.filter((todo) => !todo.isTagged))')
    expect(script).toContain('const completedTodoIds = ownDoneList.value.map((todo) => todo.id)')
    expect(template).toContain('<template #header-actions>')
    expect(template).toContain('v-if="ownDoneList.length > 0"')
    ...
    expect(ko.todoBoard.messages.deleteCompletedTitle).toContain('내가 작성한 완료된 할일')
    expect(ko.todoBoard.messages.deleteCompletedConfirm).toContain('다른 사람이 작성해 나에게 태그한 항목은 제외')
    expect(en.todoBoard.messages.deleteCompletedTitle).toContain('your completed tasks')
    expect(en.todoBoard.messages.deleteCompletedConfirm).toContain('created by others and tagged to you are excluded')
  })

  it('captures the completed IDs before confirmation and sends only that snapshot', () => {
    ...
    expect(handler).toContain('const completedTodoIds = ownDoneList.value.map((todo) => todo.id)')
