...
- [x] 사용자, 일정, Todo를 신고할 수 있고 사유·대상·신고자·시각과 신고 시점의 콘텐츠 스냅샷을 서버에 저장한다. 프로필 사진과 첨부파일은 그것이 속한 사용자·일정·Todo 신고에 포함된다. 공개 달력에서는 신고를 누르면 로그인 화면으로 유도한다.
- [x] 운영자가 신고 목록과 신고 시점 콘텐츠를 조회하고 상태 변경·콘텐츠 삭제·계정 정지를 실행할 관리자 화면을 만든다. (`/admin/reports`, `/admin/inquiries`, 회원 상세의 계정 정지·해제)
