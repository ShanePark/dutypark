...
## 1. 사용자 신고·차단 및 문의 (개발 완료, 운영 배포 대기)

App Review Guideline 1.2가 요구하는 항목이다. Dutypark는 회원 검색으로 모르는 사람에게 친구 요청을 보낼 수 있고, 공개 달력은 비로그인 상태에서도 열린다. 아래는 서버·웹·iOS에 모두 구현되었고, 운영 배포는 남아 있다.

- [x] 사용자 차단·차단 목록·차단 해제를 서버, 웹, iOS에 구현한다. 차단하면 서로의 달력·일정·일정 첨부, 회원 검색, 친구 요청과 알림이 양방향으로 끊긴다. 같은 팀 근무표는 정책상 예외로 계속 표시된다.
- [x] 사용자, 일정, Todo를 신고할 수 있고 사유·대상·신고자·시각과 신고 시점의 콘텐츠 스냅샷을 서버에 저장한다. 사용자 신고에는 프로필 사진 존재 여부·버전을, 일정·Todo 신고에는 첨부파일명을 기록한다. 공개 달력에서는 신고를 누르면 로그인 화면으로 유도한다.
- [x] 운영자가 신고 목록과 신고 시점 콘텐츠를 조회하고 상태 변경·콘텐츠 삭제·계정 정지를 실행할 관리자 화면을 만든다. (`/admin/reports`, 회원 상세의 계정 정지·해제)
- [x] 금지 콘텐츠, 제재 단계, 이의제기와 신고 후 24시간 이내 조치 기준을 담은 이용 정책을 로그인 없이 제공하도록 구현한다. 운영 배포 후 [이용약관](https://dutypark.o-r.kr/terms)의 `2026-08-18` 버전 제8~11조가 반영된다.
- [x] 문의 양식을 로그인 없이 열리는 Support URL과 iOS 더보기·게스트 화면에 제공한다. 로그인 회원은 답변 알림을 받고 앱의 `내 문의 내역`에서 답변을 확인하며, 비회원 문의에는 관리자가 입력된 이메일로 직접 회신한다. 운영자는 `/admin/inquiries`에서 문의와 사용자 공개 답변을 관리한다. (https://dutypark.o-r.kr/support)

차단의 정확한 범위와 예외는 아래와 같다. Review Notes와 App Store Connect 답변도 이 내용에 맞춘다.

- 프로필 사진 조회(`GET /api/members/{id}/profile-photo`)는 인증·차단 검사가 없는 공개 엔드포인트다. 차단해도 사진 자체는 직접 URL로 열린다.
- 차단 시 상대가 나를 태그한 일정·Todo의 태그 관계도 제거된다.
...
- [ ] Review Notes에 로그인, 소셜·Apple 로그인, 회원 탈퇴, 푸시, Universal Link, 신고·차단과 문의·답변의 재현 경로를 작성한다.
