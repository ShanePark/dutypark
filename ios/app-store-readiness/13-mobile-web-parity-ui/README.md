...
| 5 | 대시보드 · 헤더 | 로고와 액션에 불필요한 캡슐 테두리 | 수정됨·커밋 전 | 다음 기능 커밋 |
| 6 | 하단 탭 | `달력`이 `캘린더`로 변경됨 | 수정됨·커밋 전 | 다음 기능 커밋 |
| 7 | 전역 현지화 | 한국어 앱에서 기본 패턴·메뉴 등이 영어로 표시됨 | 수정됨·커밋 전 | 다음 기능 커밋 |
...
- 관련 구현은 단위 테스트와 시각 캡처를 통과했으며 다음 기능 커밋에 포함한다.
...
- [ ] 헤더·탭·Root 메뉴 현지화 기능 커밋 후 문서에 커밋 해시 반영
