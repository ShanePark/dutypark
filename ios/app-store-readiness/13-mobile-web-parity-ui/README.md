...
| 완료 단위 | 기능 커밋 | 보고 커밋 |
| --- | --- | --- |
| 관리자 회원 목록 활성 세션 문구 | `e34d67c4` | `9b09a393` |
| 숨김 근무유형이 포함된 기본 패턴 경고 | `4ba4ab07` | `1b3dc379` |
| 게스트 이용 안내의 네이티브 홈 복귀 | `6b69ce5a` | `f8f832f7` |
| 알림 드롭다운 작성자 아바타 | `0d16412b` | `323e2e18` |
| 관리자 회원 상세 상태 지표 | `ec2cbf3f` | `e3c46ec2` |
...
| 7 | 전역 현지화 | 한국어 앱에서 기본 패턴·메뉴·팀 월·요일 이름 등이 영어로 표시됨 | 완료 | `74858618`, `2cdc26ea`, `3987ea66`, `3f9fe25a` |
...
| `9925509a` | 모바일 parity 시각 캡처 흐름 추가 | `ParityVisualCaptureUITests` 1건 | 홈·Todo·메뉴·설정·패턴 기준 캡처 | 완료 |
| `58cbf120` | 달력·팀을 포함한 시각 비교 범위 확장 | `ParityVisualCaptureUITests` 1건 | 홈·Todo·달력·팀·메뉴·설정·패턴 8화면 캡처 | 완료 |
...
- 후속 `b84b2478`에서 실제 카드가 있는 fixture로 일반 탭, 0.5초 롱프레스 재정렬, 세로 스크롤, 선택 열 중앙 정렬을 iPhone 13 mini UI 5/5로 검증했다.
...
- 최초 캡처는 웹이 가입 팀, iOS가 미가입 fixture라 직접 비교할 수 없었다. 후속 `d444f095`에서 가입 팀 fixture를 추가해 연월 선택기·일정 삭제·구성원 제외를 실제 화면으로 검증했고, 이후 요일 현지화·빈 근무 영역·인원 단위까지 별도 캡처로 마쳤다.
...
아래 항목은 소스 대조로 불일치가 확인됐지만 이번 마감 범위에는 포함하지 않았다. 다음 작업은 이 순서로 RED 테스트와 동일 fixture를 먼저 만든 뒤 진행한다.

1. **관리자 회원 상세 날짜 가독성**

   iOS는 가입일·최근 활동을 서버 ISO 원문으로 표시하고 최근 수정일을 누락한다. 웹처럼 앱 locale의 날짜·시간과 활동 없음 상태를 표시하고, 최근 수정일을 추가해야 한다.
2. **설정의 개별 접속 세션 종료 문구**

   특정 세션을 종료하는 제목과 동작이 iOS에서 모두 `로그아웃`으로 축약된다. 웹의 `접속 세션 종료` / `접속 종료`로 범위를 명확히 해야 한다.
3. **친구관리 카드 정보 밀도**

   모바일 웹은 관계·고정·관리 액션 중심의 compact card지만 iOS는 팀·오늘 근무·일정까지 함께 표시한다. 동일 데이터 fixture로 카드 높이와 정보구조를 비교한 뒤 관리 화면에 필요한 정보만 남겨야 한다.

새 항목은 웹과 iOS의 동일 경로·동일 데이터 근거가 확보된 경우에만 이 목록에 추가한다. 완료 시 기능 커밋과 시각 보고 커밋을 분리하고, 진행표·커밋 보고·상세 변경 보고를 함께 갱신한다.
