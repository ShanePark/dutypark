...
- 앱의 권한 요청, 기기 token 수신, 서버 등록·해제와 Debug/Release APNs 환경 분리는 구현돼 있다.
...
- 남은 일은 구현 이력 정리가 아니라 Apple 운영 키 설정, sandbox·production 실수신, 알림 이동과 PWA 회귀 검증이다.

## 남은 체크

### Apple 및 운영 서버

- [ ] APNs Auth Key(`.p8`)를 생성하거나 기존 전용 키의 소유자와 rotation 책임자를 확정한다.
- [ ] `APNS_TEAM_ID`, `APNS_KEY_ID`, `APNS_PRIVATE_KEY`를 운영 secret manager에 등록하고 줄바꿈 손상 없이 서버가 읽는지 확인한다.
