...
| QA-002 | S3(캡처 환경) | iPhone 17 Pro Max, iOS 26.5, real local demo account | 격리 `dutypark_demo` backend 8081에서 Demo App Store capture ko/en 실행 | 기존 8080 DB 불일치로 차단됐으나 격리 backend·deterministic fixture로 해소; 기대한 local demo 로그인·현행 UI 캡처 성공 | 사용자 dev backend와 캡처 DB를 같은 8080에서 공유한 환경 결합 | API endpoint를 loopback 8080/8081로 제한하고 8081 격리 실행, marker identity guard 후 fixture reset, 영문 Calendar·Team·D-Day를 2026-11로 고정 | `/private/tmp/dutypark-appstore-capture-final3-20260829.xcresult`: 2/2 PASS | raw 14개 추출, final 12개 합성, compose/extract script PASS, 전수 시각 검토 PASS | 수정·재검증 완료 |
...
| 2026-08-29 | 격리 demo seed·App Store 캡처 | PASS | 사용자 8080/5173은 그대로 두고 `dutypark_demo` backend를 loopback 8081에서 실행. marker identity guard와 deterministic reset 후 ko/en 캡처 2/2 PASS, raw 14개·final 12개 갱신, 영문 Calendar·Team·D-Day 2026-11 및 전수 시각 검토 완료. 결과 `/private/tmp/dutypark-appstore-capture-final3-20260829.xcresult` |
